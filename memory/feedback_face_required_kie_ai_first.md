...
Type 8 Kanji-style) MUST be generated via kie.ai Nano Banana Pro (nano-banana-pro) with `image_input[0]`
...
**Correct path:**
1. kie.ai Nano Banana Pro (nano-banana-pro) with `image_input: [face_primary.blotato_url]` — PRIMARY for
...
3. Blotato Instagram Carousel Slideshow template (id `53cfec04-2500-41cf-8cc1-ba670d2c341a`)
   with `model: "nano-banana-pro"` AND face URL passed as input — only as a last resort.
...
**Updated 2026-04-25:** from the remote routine sandbox (Anthropic firewalled api.kie.ai), use the Cloudflare Worker proxy `https://api.kie.ai` instead. Same paths, same Authorization header, transparent passthrough. From local PC, hit api.kie.ai directly.
